...
/**
 * DeleteClassModal - confirmation dialog before permanently removing a class.
 * @param {boolean} isOpen - Whether the modal is visible.
 * @param {function} onClose - Closes the modal.
 * @param {boolean} busy - True while the delete request is in flight.
 * @param {Object|null} record - The class being deleted.
 * @param {function} onConfirm - async () => Promise; parent closes on success.
 */
export default function DeleteClassModal({ isOpen, onClose, busy, record, onConfirm }) {
  const name = record?.class_name ? `${record.class_name} (${record.short_code} · ${record.section})` : 'this class';

  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Delete Class" size="sm">
